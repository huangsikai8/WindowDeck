...
**The scale and the ink ratio meet in `iconSize`, and only one of them scales.** `preferredIconSize`
is a preferred size, so it scales with the setting; `inkHeadroom` is a *ratio*, so it does not — it
lives on `DeckLayout` beside `clusterWidthUnits`, for the reason stated there. Scaling a ratio applies
the scale twice. The invariant that keeps the two safe together is the same one at every scale, since
`ink = 0.898 x 1.11 x width` is a statement about a tile's width and says nothing about how that width
was arrived at. `iconInkStaysInsideItsTile` asserts it at 1x and at the extremes of the setting.
